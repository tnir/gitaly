...
func ParseObjectInfo(stdout *bufio.Reader) (ObjectInfo, error) {
	...
	if err != nil {
		return ObjectInfo{}, fmt.Errorf("read info line: %v", err)
	}

	infoLine = strings.TrimSuffix(infoLine, "\n")
	if strings.HasSuffix(infoLine, " missing") {
		return ObjectInfo{}, NotFoundError{fmt.Errorf("object not found")}
	}
	...
	if err != nil {
		return ObjectInfo{}, fmt.Errorf("parse object size: %v", err)
	}
